...
import { streamResearch, callAI } from "./ai/client";
...
export async function createTask(
  ...
  const task: TaskRun = {
    ...
    createdAt: now,
    updatedAt: now,
    session: { provider: "ai-sdk", sessionId: "", createdAt: now },
    input,
  ...
  void runTask(task, firstPrompt, agent.prompts.system, modelId).catch(console.error);
...
export async function createTaskFromSelections(
  ...
  void runTask(task, fullPrompt, agent.prompts.system, modelId).catch(console.error);
...
export async function sendUserMessage(
  ...
  void runTask(updated, content, agent.prompts.followup, modelId).catch(console.error);
}
  ...
  await streamResearch(
    ...
    systemPrompt,
    {
      ...
      onDone: (text) => {
        ...
        processAssistantOutput(task, text, agent!.output.jsonBlockTag);
      },
      onError: (err) => {
        const errTask = {
          ...task,
...
function processAssistantOutput(task: TaskRun, text: string, jsonTag: string): void {
  const hasReportJson = text.includes(`<${jsonTag}>`);
  if (hasReportJson) {
    finalizeReport(task, text, jsonTag);
    return;
  }
...
async function triggerExecution(task: TaskRun): Promise<void> {
  ...
  void runTask(reportingTask, `${executePrompt}\n\n${agent.prompts.report}`, agent.prompts.system).catch(console.error);
}

function finalizeReport(task: TaskRun, text: string, tag: string): void {
  const raw = extractJsonBlock(text, tag);
  if (!raw || typeof raw !== "object") {
    const errTask = {
      ...task,
      phase: "failed" as TaskPhase,
      status: "error" as const,
      error: { code: "parse_error", message: "报告 JSON 解析失败" },
      updatedAt: new Date().toISOString(),
    };
    store.saveTask(errTask);
    ...
    .map((c, i) => {
      ...
      if (type === "comparison")
        return {
          ...
          rows: (c.rows ?? []) as Array<Record<string, string>>,
        };
      if (type === "risk")
        return {
          type: "risk" as const,
          id: String(c.id ?? `card_${i}`),
          title: String(c.title ?? ""),
          level: (c.level ?? "medium") as "low" | "medium" | "high",
          description: String(c.description ?? ""),
          mitigation: c.mitigation ? String(c.mitigation) : undefined,
        };
      if (type === "timeline")
        return {
          ...
          id: String(c.id ?? `card_${i}`),
          title: String(c.title ?? ""),
          items: (c.items ?? []) as Array<{ label: string; description: string }>,
        };
      return {
        type: "insight" as const,
        id: String(c.id ?? `card_${i}`),
        title: String(c.title ?? ""),
        points: (c.points ?? c.key_points ?? []) as string[],
      };
    });
